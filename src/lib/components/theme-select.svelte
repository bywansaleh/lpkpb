...
<button
  class="{dark
    ? 'bg-white ring-offset-blue-700 focus:ring-gray-100'
    : 'bg-white ring-offset-blue-700 focus:ring-gray-100'} relative m-4 inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2"
  class:hidden
  type="button"
  aria-checked={dark}
  on:click={toggle}
>
  ...
  <span
    class="{dark
      ...
      : 'translate-x-4 bg-gray-400'} pointer-events-none relative inline-block h-4 w-4 transform rounded-full shadow ring-0 transition duration-200 ease-in-out"
  >
    <span
      class="{dark
